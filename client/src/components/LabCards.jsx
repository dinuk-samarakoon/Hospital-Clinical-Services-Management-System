  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        // Fetch total lab result count
        const labResponse = await axios.get("http://localhost:5000/medical-record/labReports/totalcount");
        setLabCount(labResponse.data.totalReports); // Use the correct key 'totalReports'
  ...
  return (
    <div className="flex justify-between gap-10">
      ...
      <div className="mt-5 flex px-10 py-3 items-center justify-center gap-5 bg-slate-200 rounded-lg">
        <FaUsers size={30} className="text-blue-500" />
        <div className="flex flex-col gap-2">
          <h1 className="font-semibold text-sm">Total Lab Reports Issued</h1>
          <h2 className="text-gray-700 text-sm">{labCount}</h2>  {/* Display staff count */}
        </div>
      </div>
    </div>
  );
